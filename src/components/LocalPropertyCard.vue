...
import { LISTING_STATUSES, completeness, locationOf, priceOf, titleOf } from '../models/property'
  ...
  <RouterLink :to="{ name: 'local-property', params: { id: p.id } }" class="card">
    ...
    <div class="body">
      <div class="row">
        ...
        <span class="status" :style="{ color: status.color, borderColor: status.color }">{{ t(`props.statuses.${p.status}`) }}</span>
      </div>
      <h3 class="title">{{ titleOf(p) }}</h3>
      <div class="location muted">{{ locationOf(p) || '—' }}</div>
      <ul class="specs">
        <li v-if="p.bedrooms != null">{{ t('common.rooms', { n: p.bedrooms }) }}</li>
        <li v-if="p.bathrooms != null">{{ t('common.baths', { n: p.bathrooms }) }}</li>
        <li v-if="p.builtArea">{{ p.builtArea }} m²</li>
      </ul>
      <div class="foot muted">
        <span>{{ t('common.ref') }} {{ p.ref || '—' }}</span>
        <span v-if="pendingPhotos" class="pending">● {{ t('props.detail.photosToUpload', pendingPhotos) }}</span>
