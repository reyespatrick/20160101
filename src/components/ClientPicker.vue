...
<template>
  <div class="picker">
    <div v-if="modelValue.remoteId" class="chosen">
      <span>👤 {{ modelValue.label || `Cliente nº ${modelValue.remoteId}` }}</span>
      <button type="button" aria-label="Quitar cliente" @click="clear">×</button>
    </div>
    <template v-else>
      <input v-model="text" type="search" placeholder="Nombre, teléfono o email del cliente…" autocomplete="off" />
      <p v-if="clients.searching" class="muted small">Buscando en Inmovilla…</p>
      <p v-else-if="text && !candidates.length" class="muted small">Solo se pueden enlazar clientes que ya están en Inmovilla. Por nombre se busca entre los conocidos en este dispositivo.</p>
