...
function open() {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      ...
      if (!db.objectStoreNames.contains('properties') || !db.objectStoreNames.contains('photos') || !db.objectStoreNames.contains('meta')) {
        const version = db.version + 1
        db.close()
        const again = indexedDB.open(DB_NAME, version)
        again.onupgradeneeded = () => {
          const fresh = again.result
          if (!fresh.objectStoreNames.contains('properties')) fresh.createObjectStore('properties', { keyPath: 'key' }).createIndex('agency', 'agency', { unique: false })
          if (!fresh.objectStoreNames.contains('photos')) fresh.createObjectStore('photos', { keyPath: 'key' }).createIndex('property', 'property', { unique: false })
